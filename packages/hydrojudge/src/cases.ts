...
function isValidConfig(config) {
    ...
    if (config.multi_pass && (!Number.isInteger(config.multi_pass) || config.multi_pass < 2 || config.multi_pass > 10)) {
        throw new FormatError('Multi Pass must be between 2 and 10.');
    }
