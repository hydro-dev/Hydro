...
export default class HDUOJProvider extends BasicFetcher implements IBasicProvider {
    ...
    async getProblem(id: string) {
        ...
        for (const node of problemContent.children) {
            if (node.tagName.toLowerCase() === 'font' || node.tagName.toLowerCase() === 'h1' || node.tagName.toLowerCase() === 'center' || node.innerHTML === '&nbsp;') {
                continue;
            }
    ...
    async submitProblem(id: string, lang: string, code: string, info) {
        ...
        let language = lang.includes('hduoj.') ? lang.split('hduoj.')[1] : '0';
        ...
        const { text: status } = await this.get(`/status.php?first=&pid=${id}&user=${this.account.handle}&lang=${parseInt(language) + 1}&status=0`);
