...
export class ProblemDetailHandler extends ContestDetailBaseHandler {
    ...
    async _prepare(domainId: string, pid: number | string, tid?: ObjectId) {
        ...
        if (tid) {
            if (this.tdoc?.problems?.findIndex((p) => p.pid === this.pdoc.docId) === -1) throw new ContestNotFoundError(domainId, tid);
        ...
        if (typeof this.pdoc.config !== 'string') {
            ...
            if (this.tdoc?.langs && this.tdoc?.langs.length) t.push(this.tdoc.langs);
