...
(${i.pm2_env.pm_exec_path} ${(i.pm2_env.args || []).join(' ')})`);
    ...
    const command = async () => {
        ...
        console.log(yaml.dump(infos, { schema: yaml.CORE_SCHEMA, seqNoIndent: true, lineWidth: process.stdout.columns || 80 }));
        logger.info('Uploading detail to pastebin...');
        const tmpFile = path.join(tmpdir(), 'report.yaml');
        fs.writeFileSync(tmpFile, yaml.dump(infoDetails, { seqNoIndent: true, lineWidth: 300 }));
