...
const page = new NamedPage(['contest_edit', 'contest_create', 'homework_create', 'homework_edit'], (pagename) => {
  ...
  if ($('#problem-editor').length) {
    const pidsInput = $('[name=pids]');
    const problemConfigInput = $('[name=problemConfig]');
    ReactDOM.createRoot($('#problem-editor')[0]).render(
      <ContestProblemEditor
        pids={pidsInput.val().toString().split(',').map((i) => +i)}
        problemConfig={JSON.parse(problemConfigInput.val() as string)}
        onChange={(pids, problemConfig) => {
          pidsInput.val(pids.join(','));
          problemConfigInput.val(JSON.stringify(problemConfig));
        }}
      />,
    );
  }
