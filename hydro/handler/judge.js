const
    { GET, SOCKET } = require('../service/server'),
    { assertQueue, consume } = require('../service/queue'),
    { requirePriv } = require('./tools'),
    record = require('../model/record'),
    token = require('../model/token'),
    user = require('../model/user'),
    { PRIV_JUDGE } = require('../privilege');

assertQueue('judge');

GET('/judge/noop', requirePriv(PRIV_JUDGE), async ctx => {
    ctx.body = {};
});
SOCKET('/judge/conn', async conn => {
    let sid = await new Promise((resolve, reject) => {
    ...
    let session = await token.get(sid, token.TYPE_SESSION);
    let u = await user.getById(session.uid);
    if (!u.hasPriv(PRIV_JUDGE)) conn.close(1000, 'PrivilegeError');
    let h = 1;
